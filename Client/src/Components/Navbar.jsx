...
const Navbar = ({setShowLogin}) => {
  ...
  return (
    ...
    xl:px-32 py-4 text-gray-600 border-b border-[var(--color-borderColor)] relative transition-all
    ${location.pathname === '/' && "bg-[var(--color-light)]"}`}>
     ...
     <img src={assets.logo} alt="" className='h-8' />
      ...
      max-sm:border-t border-[var(--color-borderColor)] right-0 flex flex-col sm:flex-row
      items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 transition-all
      duration-300 z-50 ${location.pathname === '/' ? "bg-[var(--color-light)]" : "bg-white"} 
      ...
      border-[var(--color-borderColor)] px-3 rounded-full max-w-56'>
      ...
      <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
        ...
        <button onClick={()=> setShowLogin(true)} 
         className='cursor-pointer px-8 py-2 bg-[var(--color-primary)]
        hover:bg-[var(--color-primary-dull)] transition-all text-white rounded-lg'>Login</button>
      </div>
    </div>
